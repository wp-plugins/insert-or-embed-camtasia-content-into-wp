...
function get_camtasia_embeder_options()
{
	$default=array(
	...
	$opt = get_option('camtasia_embeder_option', $default);
...
function camtasia_embeder_options_page()
{
...
if(isset($_POST['save']))
{
	...
	update_option('camtasia_embeder_option', $opt);
...
?>
	...
	<!--end nivo lite box-->	
	<!--QUIZ_EMBEDER START-->
	<link rel="stylesheet" id="colorbox_css" href="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL."colorbox/themes/default/colorbox.css" ;?>" />
	...
	<script type="text/javascript">
		...
		{
			var url="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL?>colorbox/themes/"+theme+"/colorbox.css";
			return url;
		}
...
<style type="text/css">
	...
	.button_box .close_button{ width:16px; height:16px; position:absolute; background:url(<?php echo WP_camtasia_EMBEDER_PLUGIN_URL?>/close_16.png) no-repeat; margin-left:369px; cursor:pointer; }
...
<div style="width: 800px; padding-left: 10px;" class="wrap">
    ...
    <h2>Insert or Embed Camtasia Content into Wordpress Trial</h2>
...
function camtasia_embeder_menu_pages()
{
add_menu_page( "Camtasia", "Camtasia" ,'manage_options', 'camtasia_content', 'camtasia_embeder_options_page');
}

add_action('admin_menu', 'camtasia_embeder_menu_pages');

function camtasia_admin_enqueue_scripts()
	{
		if(strpos($_SERVER['REQUEST_URI'], 'camtasia_content')) #to ensure that current plugin page is being shown.
		{
...
add_action( 'admin_enqueue_scripts', 'camtasia_admin_enqueue_scripts');
?>