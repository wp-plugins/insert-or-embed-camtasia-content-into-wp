...
$id3 = new getID3();
function camtasia_print_page_navi($num_records)
{
				...
				if ($num_pages != 1)
				{
					for ($i = 1; $i <= $num_pages; ++$i)
					{
						...
						{
							echo "<a href=\"media-upload.php?type=camtasia_upload&tab=camtasia&npage=$i\">$i</a>";
...
function camtasia_print_detail_form($num, $tab="upload", $file_url="", $dirname="", $file="")
{
$opt=get_camtasia_embeder_options();
$cbox_themes=camtasia_embeder_get_colorbox_themes();
global $camtasia_embeder_width;
//$numwidth = $camtasia_embeder_width;
//$numwidth = $GLOBALS['camtasia_embeder_width'];
global $camtasia_embeder_height;
//$numheight = $camtasia_embeder_height;
//$numheight = $GLOBALS['camtasia_embeder_height'];

global $id3;
$ThisFileInfo = $id3->analyze(camtasia_getUploadsPath() . $dirname . '/' . $file);
if ( ! empty( $ThisFileInfo['video']['resolution_x'] ) )
$numwidth = (int) $ThisFileInfo['video']['resolution_x'];

if ( ! empty( $ThisFileInfo['video']['resolution_y'] ) )
$numheight = (int) $ThisFileInfo['video']['resolution_y'];
	...
	<div id="upload_detail_<?php echo $num ?>" style="display:none; margin-bottom:30px; margin-top:20px;">
		...
		<!--More lightbox options	-->
			...
			<select name="size_opt_<?php echo $num ?>" id="size_opt_<?php echo $num ?>" onchange="show_hide_custom_size_area(<?php echo $num ?>)">
				<option value="camtasia_lightebox_default">Lightbox Default</option>
...
<!--original
			...
			<div id="custom_size_area_<?php echo $num ?>" style="display:none;">
			<label style="width:80px; display:inline-block;" >Height</label><input style="padding:3px 5px" type="text" name="height_<?php echo $num ?>" id="height_<?php echo $num ?>" size="3" maxlength="4" value="<?php echo $numheight ?>"" /><select name="height_type_<?php echo $num ?>" id="height_type_<?php echo $num ?>"><option value="px">px</option><option value="%">%</option></select><br />
			<label style="width:80px; display:inline-block;" >Width</label><input  style="padding:3px 5px" type="text" name="width_<?php echo $num ?>" id="width_<?php echo $num ?>" size="3" maxlength="4" value="<?php echo $numwidth ?>"" /><select name="width_type_<?php echo $num ?>" id="width_type_<?php echo $num ?>"><option value="px">px</option><option value="%">%</option></select><br />
			</div>
...
<?php
}#end camtasia_print_detail_form()


function camtasia_printInsertForm()
{
//echo "<h3>Start camtasia_printInsertForm</h3>";
	$dirs = camtasia_getDirs();
	if (count($dirs)>0)
	{
	camtasia_print_js("camtasia");
	?>
	<title>Media Library</title>
	<?php
	 $uploadDirUrl=camtasia_getUploadsUrl();
	 //START PAGIGNATION
	 ?>
	 <div style="text-align:right; padding:5px; padding-right:10px; margin:5px 20px;">
	 <?php  $bound= camtasia_print_page_navi(count($dirs)); // print the pagignation and return upper and lower bound ?>
	 </div>
	 <?php
		...
		echo "<table class='widefat'>";
			foreach ($dirs as $i=>$dir):
				...
				echo '<tr id="content_item_'.$i.'" class="'; if ($i % 2 == 1)echo 'alternate '; echo 'iedit">
						...
						echo '</span>';
						echo '</div>';
						camtasia_print_detail_form($i, "camtasia" , $uploadDirUrl.$dir."/".$file, $dir, $file);
						echo '
						</td>
					 </tr>';
...
//echo "<h3>End camtasia_printInsertForm</h3>";
}

function camtasia_getUploadsPath()
{
$dir = wp_upload_dir();
return ($dir['basedir'] . "/".WP_camtasia_EMBEDER_UPLOADS_DIR_NAME."/");
}
function camtasia_getUploadsUrl()
{
$dir = wp_upload_dir();
return $dir['baseurl'] . "/".WP_camtasia_EMBEDER_UPLOADS_DIR_NAME."/";
}
function camtasia_getPluginUrl()
{
//return WP_PLUGIN_URL."/insert-or-embed-camtasia-content-into-wordpress/"; #oneTarek says: This line is wrong because you are unable to rename the plugin directory
...
function camtasia_getDirs()
{
$myDirectory = opendir(camtasia_getUploadsPath());
$dirArray = array();
$i=0;
// get each entry
while($entryName = readdir($myDirectory)) {
	if ($entryName != "." && $entryName !=".." && is_dir(camtasia_getUploadsPath().$entryName)):
	$dirArray[$i]['dir'] = $entryName;
	// store the filenames - need to iterate to get story.html or player.html
	$dirArray[$i]['file'] = camtasia_getFile(camtasia_getUploadsPath().$entryName);
	$i++;
	endif;
}
...
function camtasia_has_CPLibraryAll_css($dir)
{
...
function camtasia_getFile($dir)
{
$myDirectory = opendir($dir);
$fileList = array();
$camtasia_UploadsPath=camtasia_getUploadsPath();
	...
	#end Get file list
	closedir($myDirectory);
	#check for camtasia.css
	$has_camtasia_css=true;
	foreach($fileList as $fname){ if($fname== "config_xml.js"){$has_camtasia_css=true; break; } }

	if($has_camtasia_css)
	{
		//echo "has camtasia_css<br>";
		...
		else
		{
			...
			if ($ext == "htm" || $ext == "html")
				...
				//"<pre>"; print_r( $entryName); echo "</pre>";
				return $entryName;
	...
	{
		//echo "has not has_camtasia_css<br>";
		if(camtasia_has_CPLibraryAll_css($dir))
		{
			#check for index.html or index.htm
			$has_index=false;
...
function camtasia_print_js($tab="upload") #added by oneTarek
{
wp_enqueue_script("jquery");

?>
<link rel="stylesheet" href="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL."css/style.css?var=";echo time(); ?>" />
<script src="<?php echo camtasia_getPluginUrl()."js/jquery.form.js";?>" ></script>
<script>
	...
	*/
			...
			success : function(data) {
			 ...
			 else
			 {
				  if (data[0] == "uploaded")
				  {
					...
					jQuery("#width_1").val(data[4]);
					jQuery("#height_1").val(data[5]);
					<?php if($tab=="upload"){?>
					var regex = new RegExp('_', 'g');
					jQuery("#title").val(uploaded_dir_neme.replace(regex," "));
					<?php }?>
					show_detail(1);
					//win.send_to_editor("[camtasia_loader width='100%' height='600' frameborder='0' src='"+dir+"']");
			...
			if( filename.slice(-3) != "zip" && filename.slice(-3) != "mp4" ) {
				alert("Please choose a zip archive"); return false;
			}
...
function add_to_post(number)
{
	<?php if($tab=="upload"){?>
	  ...
	  {
	  camtasia_rename_dir(old_name, new_name);
	  }
	  else
	  {
...
{
<?php $opt=get_camtasia_embeder_options();?>
...
//alert(insert_as);
	 ...
	 case 2:
	  ...
	  shortCodeOptions=" href='"+uploaded_file_url+"'";
	  	var more_lightbox_option= parseInt(jQuery('input[name=more_lightbox_option_'+number+']:checked').val());
			...
			if(size_opt=="camtasia_lightebox_default")
			{
				var w=parseInt(jQuery("#width_"+number).val());
				var wt=jQuery("#width_type_"+number).val();
				var h=parseInt(jQuery("#height_"+number).val());
	...
	shortCode="[camtasia_iframe_loader type='"+shortCodeType+"' " +shortCodeOptions+"]";
	win.send_to_editor(shortCode);

}// end insert_into_post()

function camtasia_rename_dir(old_name, new_name)
{

var loading_text='<img src="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Loading" /> Saving....';
	jQuery('#insert_msg_1').html(loading_text);


			jQuery.getJSON("<?PHP bloginfo('url') ?>/wp-admin/admin-ajax.php?action=camtasia_rename_dir&dir_name="+old_name+"&title="+new_name, function(data) {
				...
				if(data[0]=="success")
				{
				 ...
				 jQuery('#file_url_1').val("<?php echo camtasia_getUploadsUrl();?>"+new_renamed_dir_name+"/"+old_file_name);
...
function delete_dir(number)
{
	...
	var loading_image='&nbsp;&nbsp;<img src="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Launch Presentation" />&nbsp;Deleting..'
	var loading_text='<img src="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL;?>loading_16x16.gif" alt="Loading" /> Deleting....';
	...
	if(dir_name!="")
	{
		...
		jQuery.post("admin-ajax.php",{dir : dir_name,action:'camtasia_del_dir'},function(data){
			//alert("Deleted");
			<?php if($tab=="upload"){?>
...
<?php
}#end function camtasia_print_js()

function camtasia_print_upload()
{
// echo "<h3>Start camtasia_print_upload</h3>";
camtasia_print_js();
?>
<?PHP
...
if (count($dirs)>1)
{
echo '<span style="color:#ff0000; font-size:15px;">The trial version only supports two uploads. Please purchase the full version with unlimited uploads at <a href="http://www.elearningplugins.com/products/insert-embed-camtasia-content-wordpress-plugin/" target="_blank">www.elearningplugins.com</a></span>';	
}
else
{
?>
<form enctype="multipart/form-data" id="myForm1" action="admin-ajax.php" method="POST">
<input type="hidden" name="action" value="camtasia_upload" />
...
<p><i>Please choose a MP4 or .zip file that you published with the Camtasia software</i></p>
<img id="media_loading" style='display:none;' src= "<?php echo camtasia_getPluginUrl() . "loading.gif" ;?>" /><br />
<?php
 	camtasia_print_detail_form(1);
}
?>


<p><a href="http://www.elearningplugins.com/products/insert-embed-camtasia-content-wordpress-plugin/" target="_blank">Need help?</a> | Maximum upload file size on your server: <strong><?php echo $upload_size_unit; echo $sizes[$u]; ?></strong> | <a href="http://www.elearningplugins.com/increase-maximum-upload-file-size/" target="_blank">Increase?</a>
<p/>
...
<?php
 //echo "<h3>END camtasia_print_upload</h3>";
}



function wp_ajax_camtasia_del_dir()
{
$dir = camtasia_getUploadsPath().$_POST['dir'];
camtasia_rrmdir($dir);
die();
}


function wp_ajax_camtasia_rename_dir()
{
$arr=array();
	if(isset($_REQUEST['dir_name']) && $_REQUEST['dir_name']!="")
	{
	$target = camtasia_getUploadsPath().$_REQUEST['dir_name'];
		if(file_exists($target))
		{
			...
			$title=trim($_REQUEST['title']);
				if($title)
				{
					...
					$new_file=camtasia_getUploadsPath().$title;
...
function wp_ajax_camtasia_check_folder($dir){
	...
	if(count($structure) == 1){
		...
		wp_ajax_camtasia_check_folder($dir);
	}
	
}

function wp_ajax_camtasia_upload()
{
$arr = array();
...
$target = camtasia_getUploadsPath().$dir[0];
$index = count($dir) -1;
$mp4file = ($dir[$index] == "mp4");

	if (!isset($dir[$index]) || ($dir[$index] != "zip" && $dir[$index] != "mp4") )
		{
		$arr[0] = "the file must be zip archive or MP4 file";
		}
	else
		{
			...
			if (!empty($file))
			$arr = camtasia_extractZip($file,$target,$dir[0],$mp4file);
			else
		...
		//$arr[0] = $arr[1];
global $camtasia_embeder_width;
$camtasia_embeder_width = (int) $arr[4];
global $camtasia_embeder_height;
$camtasia_embeder_height = (int) $arr[5];
$GLOBALS['camtasia_embeder_width'] = (int) $arr[4];
$GLOBALS['camtasia_embeder_height'] = (int) $arr[5];
echo json_encode($arr);
//$arr[0] = json_encode($arr);
die();
}
function camtasia_extractZip($fileName,$target,$dir,$mp4file)
{
		$arr = array();
		if(!$mp4file) {
			$zip = new ZipArchive;
			...
			$zip->close();
		$file = camtasia_getFile($target);
		} else {
			if ( ! is_dir($target)) {
				mkdir($target);
			}
			move_uploaded_file($fileName, $target . '/video.mp4');
			$res = true;
			$file = true;
		}
     ...
     if ($res === TRUE) {
		...
		if($file)
		{
		 $arr[0] = 'uploaded';
		 if($mp4file) {
		 	$arr[1] = camtasia_getUploadsUrl() . $dir . '/video.mp4';
		 } else {
		 	$arr[1] = camtasia_getUploadsUrl().$dir."/".$file;
		 }
		 $arr[2] = $dir;
		 $arr[3] =$file;
		 $arr[4] = 'width';
		 ...
		 // try to get mp4 dimensions
				  ...
				  //$id3 = new getID3();
				  if($mp4file) {
				  	$ThisFileInfo = $id3->analyze($target . '/video.mp4');
				  } else {
				  	$ThisFileInfo = $id3->analyze($arr[6]);
				  }

				  if ( ! empty( $ThisFileInfo['video']['resolution_x'] ) )
				  $arr[4] = (int) $ThisFileInfo['video']['resolution_x'];
				  global $camtasia_embeder_width;
				  $camtasia_embeder_width = $arr[4];
				  $GLOBALS['camtasia_embeder_width'] = $arr[4];
				  if ( ! empty( $ThisFileInfo['video']['resolution_y'] ) )
				  $arr[5] = (int) $ThisFileInfo['video']['resolution_y'];
				  global $camtasia_embeder_height;
				  $camtasia_embeder_height = $arr[5];
				  $GLOBALS['camtasia_embeder_height'] = $arr[5];
		}
		 else
			 {
			 $arr[0] ="Please upload Camtasia content or verify that the content was properly zipped";
			 camtasia_rrmdir($target);
			 }
...
function camtasia_rrmdir($dir) {
   if (is_dir($dir)) {
     $objects = scandir($dir);
     foreach ($objects as $object) {
       if ($object != "." && $object != "..") {
         if (filetype($dir."/".$object) == "dir") camtasia_rrmdir($dir."/".$object); else unlink($dir."/".$object);
       }
     }
     reset($objects);
     rmdir($dir);
...
function camtasia_embeder_wp_head()
{
$opt=get_camtasia_embeder_options();
global $camtasia_embeder_size_opt;
global $camtasia_embeder_width;
global $camtasia_embeder_height;
global $camtasia_embeder_scrollbar;
if(isset($camtasia_embeder_size_opt))
	{
	if($camtasia_embeder_size_opt=="custom_form_dashboard"){$camtasia_embeder_width=$opt['width'].$opt['width_type']; $camtasia_embeder_height=$opt['height'].$opt['height_type'];}
	}
...
?>
	<!--nivo lite box-->
	<link rel="stylesheet" href="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL?>nivo-lightbox/nivo-lightbox.css" type="text/css" />
	<link rel="stylesheet" href="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL?>nivo-lightbox/themes/default/default.css" type="text/css" />
	<?php if($camtasia_embeder_size_opt!="camtasia_lightebox_default"){ ?>
	<style type="text/css">
	.nivo-lightbox-content {
	width: <?php echo $camtasia_embeder_width; ?>;
	height: <?php echo $camtasia_embeder_height; ?>;
	margin:auto;
	}
		<?php if(isset($camtasia_embeder_scrollbar) && $camtasia_embeder_scrollbar=="no"){?>
	...
	<?php }?>
	<script src="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL?>nivo-lightbox/nivo-lightbox.min.js"></script>
	<!--end nivo lite box-->
<?php } else{
	global $camtasia_embeder_colorbox_theme; if(!isset($camtasia_embeder_colorbox_theme) || $camtasia_embeder_colorbox_theme==""){$camtasia_embeder_colorbox_theme=$opt["colorbox_theme"];}
	//if($camtasia_embeder_size_opt!="custom_form_dashboard" && $camtasia_embeder_size_opt!="custom"){ $camtasia_embeder_width="80%"; $camtasia_embeder_height="80%";}//default
	//if($camtasia_embeder_size_opt!="custom_form_dashboard" && $camtasia_embeder_size_opt!="custom"){ $camtasia_embeder_width="80%"; $camtasia_embeder_height="80%";}//default
?>
	<!--camtasia_EMBEDER START-->
	<link rel="stylesheet" href="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL."colorbox/themes/".$camtasia_embeder_colorbox_theme."/colorbox.css" ;?>" />
	<script type="text/javascript" src="<?php echo WP_camtasia_EMBEDER_PLUGIN_URL."colorbox/jquery.colorbox-min.js" ;?>" ></script>
<?php } ?>
	...
	<script type="text/javascript">
		jQuery(document).ready(function($){
			...
			$(".colorbox_iframe").colorbox({iframe:true, transition:"<?php echo $opt['colorbox_transition']?>", width:"<?php echo $camtasia_embeder_width ?>", height:"<?php echo $camtasia_embeder_height ?>", scrolling:<?php echo($camtasia_embeder_scrollbar=="no")?'false':'true'; ?>});
			<?php }?>
...
function camtasia_embeder_get_colorbox_themes()
{
	$themes=array(
...
function camtasia_embeder_admin_scripts()
 {
	 if (isset($_GET['page']) && $_GET['page'] == 'camtasia_content')
	 {
	 wp_enqueue_script('jquery');
	 wp_enqueue_media();
	 wp_register_script('camtasia_embeder_upload', WP_camtasia_EMBEDER_PLUGIN_URL.'js/admin.js');
	 wp_enqueue_script('camtasia_embeder_upload');
	 }
 }
 add_action( 'admin_enqueue_scripts', 'camtasia_embeder_admin_scripts' );
?>
